...
export default function HomeWhyUsSection() {
  ...
  return (
    <div className="bg-[#33404D] w-max-10xl text-white py-25 px-6 text-center">
      {/* العنوان */}
      <div className="mb-12">
        <div className="flex justify-center mb-2">
          <img src={fire} className="text-yellow-500 w-[60px]" />
        </div>
        <h4 className="uppercase text-[#E9DFC9] text-[18px] font-semibold mt-5">Why Us</h4>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6  max-w-5xl mx-auto">
        {services.map((service, idx) => (
          <div
            key={idx}
            className="group p-10 rounded-2xl   hover:shadow-2xl  hover:bg-gray-700 transition duration-400  shadow-lg flex flex-col xl:items-start xl:text-start items-center text-center"
          >
            <img src={service.icon} alt="" className="w-[60px] " />
            <h3 className="mt-4 font-bold text-[20px]">{service.title}</h3>
            <p className="mt-2 text-[16px] text-gray-300">{service.desc}</p>
          </div>
        ))}
      </div>
    </div>
